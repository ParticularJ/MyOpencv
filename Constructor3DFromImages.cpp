// Constructor3DFromImages.cpp: 定义控制台应用程序的入口点。
//
...
)
{
	...
	descriptor_for_all.clear();create
	Mat image;

	//读取图像，获取图像特征点，并保存
	Ptr<Feature2D> surf = xfeatures2d::SURF::create(100,4,3);
...
int main()
{
	string img1 = "image/IBM001.png";
	string img2 = "image/IBM002.png";
	...
	//保存为YML文件格式，把该文件和SfMViewer.exe放在一起，运行SfMViewer.exe，
	//就可以看到三维模型的效果
	save_structure("image//structure.yml", rotations, motions, structure, c1);

	//system("image//SfMViewer.exe");
}
